        ...
        <div class="entry-header-inner section-inner medium">
            ...
            <div class="entry-categories">
                ...
                <div class="entry-categories-inner">
                    <a href="{{ route('category',$category_page->slug) }}"
                        rel="category tag">{{ $category_page->name }}</a>
                </div>

            </div>

            <h2 class="entry-title heading-size-1"><a
                    href="{{ route('single' ,$article->article->slug ) }}">{{ $article->article->title }}</a>
            </h2>
                ...
                <ul class="post-meta">
                    <li class="post-author meta-wrapper">
                        ...
                        <span class="meta-text">
                            Yazar <a
                                href="{{ route('single' ,$article->article->slug ) }}">{{ $article->article->user_guid }}</a>
                        </span>
                    </li>
                    <li class="post-date meta-wrapper">
                        ...
                        <span class="meta-text">
                            <a
                                href="{{ route('single' ,$article->article->slug ) }}">{{ $article->article->created_at->diffForHumans() }}</a>
                        </span>
                    </li>
                    <li class="post-comment-link meta-wrapper">
                        ...
                        <span class="meta-text">
                            <a href="{{ route('single' ,$article->article->slug ) }}#respond">
                                @if (count($article->article->article_comments)>0)
                                {{ count($article->article->article_comments) }} Yorum
                                @else
                                Yorum yok
                                @endif
                                <span class="screen-reader-text">{{ $article->article->title }}</span></a> </span>
                    </li>
                </ul>
            </div>
        </div>
...
{{ $articlesCategory->links('front.layouts.pagination',['paginator'=>$articlesCategory]) }}
@endsection
